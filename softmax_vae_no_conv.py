...
class TraditionalVAE(nn.Module):
    def __init__(self, input_dim, latent_dim, output_dim):
        ...
        self.encoder = TraditionalEncoder(input_dim, latent_dim)
        self.decoder = Decoder(latent_dim, output_dim)
...
vae_model = VAE(input_dim, latent_dim, output_dim).to(device)
vae_optimizer = optim.Adam(vae_model.parameters(), lr=learning_rate)
vae_checkpoint = "vae_checkpoint.pth.tar"

traditional_model = TraditionalVAE(input_dim, latent_dim, output_dim).to(device)
...
for epoch in range(start_epoch_vae, total_epochs):
    ...
    with tqdm(total=len(trainloader), desc=f'Epoch {epoch+1}/{total_epochs}', unit='batch') as pbar:
        for images, _ in trainloader:
            ...
            loss_vae = custom_loss_function(reconstructed_x_vae, images, epoch, total_epochs)
            loss_vae.backward()
            vae_optimizer.step()
            ...
            loss_traditional = traditional_loss_function(reconstructed_x_traditional, images, mu_traditional, logvar_traditional, epoch, total_epochs)
...
condition = torch.ones(latent_dim).to(device)  # Initialize with ones
condition[2] = 10.0  # Boosting the third category (index 2)
condition = condition.unsqueeze(0)  # Add batch dimension

conditional_z_vae = conditional_probability(encoded_z_vae, condition)
